...
public class Utility {
    ...
    public static void writeType(RandomAccessFile raf, TagType type) throws IOException {
    ...
    public static void writeString(RandomAccessFile raf, String value) throws IOException {
        if (value == null) {
            raf.writeInt(0);
            return;
        }
    ...
    public static void writeObject(RandomAccessFile raf, TagObject object) throws Exception {
    ...
    protected static void writeValue(RandomAccessFile raf, Object value, TagType type) throws Exception {
        if (type == TagType.STRING) {
            ...
            if (value instanceof TagObject) {
                ...
                writeObject(raf, new SerializationContext(value).toTagObject());
            }
            ...
            if (value instanceof TagList) {
                ...
                TagType types = getType(list.getFirst().getAsOrigin());
                writeType(raf, types);
                raf.writeInt(list.size());
                for (NBTElement tag : list) {
                    writeValue(raf, tag.getAsOrigin(), types);
                }
    ...
    public static TagType readType(RandomAccessFile raf) throws IOException {
        return TagType.values()[raf.readByte()];
    }

    public static TagObject readObject(RandomAccessFile raf) throws IOException {
